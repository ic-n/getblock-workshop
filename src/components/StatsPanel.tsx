        ...
        <Panel title="Stats">
            ...
            <Section label="Averages">
                <MetricRow
                    label="Hold time"
                    value={
                        stats.avgHoldTimeSec !== null
                            ? `${stats.avgHoldTimeSec.toFixed(1)}s`
                            : '—'
                    }
                />
                <MetricRow
                    label="NFTs / holder"
                    value={
                        stats.avgNftsPerHolder > 0
                            ? stats.avgNftsPerHolder.toFixed(2)
                            : '—'
                    }
                />
                <MetricRow
                    label="Tokens / holder"
                    value={
                        stats.avgTokensPerHolder > 0
                            ? fmtTokens(Math.floor(stats.avgTokensPerHolder))
                            : '—'
                    }
                />
            </Section>
            ...
            {snapshot && (
                <Section label="Market">
                    ...
                    <MetricRow
                        ...
                        highlight={
                            soldOut ? 'text-violet-400' : 'text-emerald-400'
                        }
                    />
                </Section>
            )}
...
function EventRow({
    ...
    return (
        <div className="flex items-center gap-2">
            <span className={`${color} w-3 shrink-0 text-center text-[11px]`}>
                {icon}
            </span>
            ...
            <span className="w-8 shrink-0 text-right text-xs text-zinc-300 tabular-nums">
                {count}
            </span>
        </div>
    );
