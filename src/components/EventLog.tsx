...
function formatEvent(e: NFTEvent): React.ReactNode {
    ...
    switch (e.type) {
        case 'MINT':
            return (
                <>
                    <span className={color}>{to}</span> minted {nft}
                </>
            );
        case 'LIST':
            return (
                <>
                    <span className={color}>{from}</span> listed {nft}
                </>
            );
        case 'UNLIST':
            return (
                <>
                    <span className={color}>{from}</span> unlisted {nft}
                </>
            );
        case 'PURCHASE':
            return (
                <>
                    <span className={color}>{to}</span> bought {nft}
                </>
            );
        case 'TRANSFER':
            return (
            ...
            {combined.length === 0 ? (
                ...
                combined.map((e, i) => {
                    ...
                    return (
                        ...
                        >
                            <span
                                className={`${color} mt-0.5 w-3 shrink-0 text-center text-xs`}
                            >
                                {icon}
                            </span>
